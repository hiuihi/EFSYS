        ...
        <div class="container" style="margin-top: 40px;">
            <div class="shop_order">
                <div class="layui-card">
                    <div class="layui-card-header" style="font-weight: 800;">收货人信息<button  class="btn_addaddress">添加新收货地址</button></div>
                                ...
                                <div class="adress_choice">
                                    <c:forEach items="${addressList}" var="address">
                                        <div  class="base" >
                                            ...
                                            <button class="btn-name" value="${address.userAddressName}">
                                                    ${address.userAddressName}
                                                <c:choose>
                                                    <c:when test="${address.province=='黑龙江省'}">
                                                        ${fn:substring(address.province, 0, 3)}
                                                    </c:when>
                                                    <c:otherwise>
                                                        ${fn:substring(address.province, 0, 2)}
                                                    </c:otherwise>
                                                </c:choose>
                                            </button>
                                            <i class="address_name">${address.userAddressName}</i>
                                            <i class="province" >${address.province}</i>
                                            <i class="city" >${address.city}</i>
                                            <i class="county" >${address.county}</i>
                                            <i class="address" >${address.address}</i>
                                            <i class="user_phone" >${address.tel}</i>
                                            <c:if test="${address.isDefault==1}">
                                                <span class="user_sited" >默认地址</span>
                                            </c:if>
                                            <p class="user_del" ><a href="javascript:void(0)"> 删除</a></p>
                                            <p class="user_edit" ><a href="javascript:void(0)" >编辑</a></p>
                                            <p class="user_site" ><a href="javascript:void(0)"> 设为默认地址</a></p>
                                        </div>
                                    </c:forEach>
                ...
                <div class="shop_list" > <span >商品清单</span>    </div>
                <!-- 细横线 -->
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="shoping__cart__table" style="width: 95%;margin: 0 auto;margin-top: 20px;">
                            <table>
                                <thead>
                                <tr>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="shoping__cart__item">
                                            ...
                                            <h5>${cart.goodsName}<br><span style="font-size: 10px;">规格: ${cart.spec}</span></h5>
                                        </td>
                                        <td class="shoping__cart__price">
                                            <c:if test="${cart.promotionPrice>0}">
                                                ￥<span>${cart.promotionPrice}</span>
                                            </c:if>
                                            <c:if test="${cart.promotionPrice==0}">
            ...
            <div class="row">
                <div class="col-lg-5 offset-lg-7">
                    <div class="cart__total__procced2" >
                        <h6>应付金额:￥
                            <c:set value="0" var="sum" />
                            <c:forEach items="${cartList}" var="cart">
                                <c:set value="${cart.totalPrice}" var="total" />
                        ...
                        <ul>
                            <li>寄送至:
                                <c:forEach items="${addressList}" end="0" var="address">
...
<%--                        <a href="../alipay/goAlipay" class="primary-btn" id="submit_orders">提交订单</a>--%>
                        <a href="javascript:void(0)" class="primary-btn" id="submit_orders">提交订单</a>
<%--                        <a href="../returnPay/testPay" class="primary-btn">提交订单</a>--%>
<%--                        <a href="javascript:void(0)" class="primary-btn" onclick="doComment()">提交订单</a>--%>
                    </div>
                </div>
            </div>
        </div>
...
<%--                <form class="" action="">--%>
                    ...
                    <div class="layui-form-item" >
                        ...
                        <div  id="distpicker2" data-toggle="distpicker">
                            <div class="layui-input-inline " style="margin-left: 29px;width: 140px;">
                                <select class="select-city" data-province="---- 选择省 ----" name="city"  id="upd-province"></select>
                            </div>
                            <div class="layui-input-inline " style="width: 145px;">
                                <select class="select-city" data-city="---- 选择市 ----" name="city"  id="upd-city"> </select>
                            </div>
                            <div class="layui-input-inline " style="width: 140px;">
                    ...
                    <div class="layui-form-item" style="margin-top: 60px;">
                        <div class="layui-input-block">
                            <button class="layui-btn layui-btn-danger" style="background-color: #7fad39;margin-left: 150px;margin-top: -50px;" lay-submit lay-filter="formDemo" id="upd-btn-danger">确认</button>
<%--                            <button type="reset" class="layui-btn layui-btn-primary">重置</button>--%>
                        </div>
                    </div>
<%--                </form>--%>
...
<script>
    ...
    //     });
    // }
    $('#submit_orders').on('click',function () {
        var totalPrice=$('.cart__total__procced2').find('span').eq(0).text();
        var address=$('.adress_choice').find('button');
        var addressId="";
        address.each(function () {
            if ($(this).is('.btn-name-select')){
        ...
        if (addressId=="" || addressId==null){
            ...
            window.location.href="../alipay/goAlipay?totalPrice="+totalPrice+"&addressId="+addressId;
        }
            ...
            $.ajax({
                ...
                success: function(obj){
                    if(obj.state==0){
                        ...
                        setTimeout( 'window.location.href="../order/showOrderConfirmation.do"',200);
                    }
                },
                //请求失败
                error:function()
...
<script>
    //点击user_site（设为默认地址）将某一地址设为默认地址
    ...
    $(".user_edit").on('click',function(){
        ...
        var sited=$(this).parent().find(".user_sited").length;
